...
let item_desc = ["Тарілка з принтом Марса. Доступна в червоному та білому кольорах", "Кружка з принтом Марса. Доступна в червоному та білому кольорах", "Масштабна модель однієї з ракет Galaxy", "Масштабна модель марсохода (ми купуємо їх у NASA)"]
for (i = 0; i<item_desc.length; i += 1) {
    item_descr_objects[i].innerHTML = item_desc[i]
}


let item_price_objects= document.querySelectorAll('.merch-item-price')
let item_price = ["1000", "1000", "2500", "5000"]
for (i = 0; i<item_price.length; i += 1) {
...
let card_btn= document.querySelectorAll('.add-to-cart-btn')
for (i = 0; i<card_btn.length; i += 1) {
    card_btn[i].addEventListener("click", add_items)
}
...
// Підрахунок вартості квитка
 le.borderBottom = '1px solid #5D4229'
       alert(total)
   }
}
